...
def show_loop(pipe_child):
    ...
    while True:
        ...
        #if not ret:
        #if np.shape(from_queue) == ():
        #    break

        frame_rgb = cv2.cvtColor(from_queue, cv2.COLOR_BGR2RGB)
        ...
        detections = multiprocessing.Process(target=darknet.detect_image,args=(netMain, metaMain, darknet_image, ))
        detections.start()
        detections.join()
